...
describe("BlackHoles", function () {
  ...
  beforeEach(async function () {
    ...
    blackHoles = Planets__factory.connect(BlackHoles.address, signers[0]) as Planets
    mintPrice = await blackHoles._price()
  })

  it("Should have the correct price set in the constructor", async function () {
    expect(await blackHoles._price()).to.equal(ethers.utils.parseEther("0.0042"))
  })
